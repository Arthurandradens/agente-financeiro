...
async function backfillPaymentMethods() {
  ...
  try {
    ...
    const totalToUpdate = await db
      .select({ count: sql`count(*)` })
      .from(transactions)
      .where(isNull(transactions.paymentMethodId));

    console.log(
      `📊 Total de transações sem payment_method_id: ${totalToUpdate[0].count}`,
    );

    if (totalToUpdate[0].count === 0) {
      console.log(
        "✅ Todas as transações já possuem payment_method_id. Backfill desnecessário.",
      );
      return;
    }
    ...
    // 1. PIX
    const pixResult = await db
      .update(transactions)
      .set({ paymentMethodId: 1 })
      .where(
        sql`payment_method_id IS NULL AND (
    ...
    const cartaoCreditoResult = await db
      .update(transactions)
    ...
    console.log(
      `✅ Cartão Crédito: ${cartaoCreditoResult.length} transações atualizadas`,
    );

    // 3. Cartão Débito
    const cartaoDebitoResult = await db
      .update(transactions)
    ...
    console.log(
      `✅ Cartão Débito: ${cartaoDebitoResult.length} transações atualizadas`,
    );

    // 4. Boleto
    const boletoResult = await db
      .update(transactions)
    ...
    // 5. TED
    const tedResult = await db
      .update(transactions)
      .set({ paymentMethodId: 2 })
      .where(
        sql`payment_method_id IS NULL AND UPPER(meio_pagamento) LIKE '%TED%'`,
      )
      .returning({ id: transactions.id });
    ...
    const docResult = await db
      ...
      .where(
        sql`payment_method_id IS NULL AND UPPER(meio_pagamento) LIKE '%DOC%'`,
      )
      .returning({ id: transactions.id });
