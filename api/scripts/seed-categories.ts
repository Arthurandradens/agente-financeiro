...
function inferCategoryKind(
  categoryName: string,
  subcategoryName?: string,
): "spend" | "income" | "transfer" | "invest" | "fee" {
  ...
  if (
    cat.includes("serviços financeiros") ||
    cat.includes("servicos financeiros") ||
    sub.includes("tarifa") ||
    sub.includes("taxa")
  ) {
    return "fee";
...
async function seedCategories() {
  ...
  try {
    ...
    console.log(
      `📊 Encontradas ${uniqueCategories.length} combinações categoria/subcategoria`,
    );
    ...
    for (const row of uniqueCategories) {
      ...
      if (
        subcategoryName &&
        !subcategories.has(`${categoryName}|||${subcategoryName}`)
      ) {
    ...
    for (const [categoryName, categoryData] of parentCategories) {
      try {
        ...
        console.log(
          `✅ Categoria pai inserida: ${categoryName} (ID: ${inserted.id})`,
        );
      } catch (error: any) {
        if (error.code === "SQLITE_CONSTRAINT_UNIQUE") {
          ...
          if (existing.length > 0) {
            ...
            console.log(
              `⚠️  Categoria pai já existe: ${categoryName} (ID: ${existing[0].id})`,
            );
          }
        } else {
          throw error;
    ...
    for (const [key, subcategoryData] of subcategories) {
      ...
      if (!parentId) {
        console.log(
          `⚠️  Categoria pai não encontrada para subcategoria: ${subcategoryName}`,
        );
        continue;
      }
      ...
      try {
        ...
        console.log(
          `✅ Subcategoria inserida: ${subcategoryName} (ID: ${inserted.id}, Pai: ${categoryName})`,
        );
      } catch (error: any) {
        if (error.code === "SQLITE_CONSTRAINT_UNIQUE") {
