...
const transactionsRoute: FastifyPluginAsync = async (fastify) => {
      ...
      try {
        ...
        if (filters.page) {
          ...
          if (isNaN(page) || page < 1) {
            throw new HttpError(
              400,
              "Parâmetro page deve ser um número positivo",
            );
          }
          filters.page = page;
        }
        ...
        if (filters.subcategoryId) {
          ...
          if (isNaN(subcategoryId)) {
            throw new HttpError(
              400,
              "Parâmetro subcategoryId deve ser um número",
            );
          }
        ...
        if (filters.paymentMethodId) {
          ...
          if (isNaN(paymentMethodId)) {
            throw new HttpError(
              400,
              "Parâmetro paymentMethodId deve ser um número",
            );
          }
  ...
  fastify.post(
    ...
    async (request, reply) => {
      try {
        console.log(
          "\n\n 📝 Recebendo transação: aaaaaaaaaaaaaaaaaaaaaaaa",
          request.body,
        );
        ...
        const transactionResult = await fastify.db
          .insert(transactions)
          .values({
            date: data.date,
  ...
  fastify.delete(
    ...
    async (request) => {
      try {
        ...
        // Excluir transação
        await fastify.db.delete(transactions).where(eq(transactions.id, id));
