...
function parseDateNubankToISO(dateStr: string | null | undefined): string | null {
...
function parseDateBradescoToISO(dateStr: string | null | undefined): string | null {
  if (!dateStr) return null
  const m = /^(\d{2})\/(\d{2})\/(\d{4})$/.exec(String(dateStr).trim())
  if (!m) return null
  const [, d, mth, y] = m
...
export class ClassificationService {
  ...
  async classifyCSV(csvContent: string, systemPrompt: string, bankId: number): Promise<any[]> {
    // Detectar formato
