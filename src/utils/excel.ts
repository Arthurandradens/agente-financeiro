...
import type { ExcelData, Transaction, CategorySummary, OverviewSummary } from '@/types'
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const requiredSheets = ['Transações', 'Resumo por categoria', 'Visão geral']
        const missingSheets = requiredSheets.filter(sheet => !workbook.SheetNames.includes(sheet))
        
        if (missingSheets.length > 0) {
          throw new Error(`Abas obrigatórias não encontradas: ${missingSheets.join(', ')}`)
        }
        
        // Ler aba de Transações
        const transacoesSheet = workbook.Sheets['Transações']
        const transacoesData = XLSX.utils.sheet_to_json(transacoesSheet, { header: 1 })
        ...
        const transacoes: Transaction[] = transacoesData.slice(1)
          .map((row: any[]) => {
            const obj: any = {}
          ...
          .filter((row: any) => {
            ...
            return row.data && row.tipo && typeof row.valor === 'number' && row.categoria
          })
        ...
        const resumoPorCategoria: CategorySummary[] = resumoData.slice(1)
          .map((row: any[]) => {
            const obj: any = {}
        ...
        const visaoGeral: OverviewSummary = {
          total_entradas: Number(visaoRow[visaoHeaders.indexOf('total_entradas')]) || 0,
          total_saidas: Number(visaoRow[visaoHeaders.indexOf('total_saidas')]) || 0,
          saldo_final_estimado: Number(visaoRow[visaoHeaders.indexOf('saldo_final_estimado')]) || 0
        }
...
export const loadFromLocalStorage = (): { data: ExcelData, fileName: string } | null => {
  try {
